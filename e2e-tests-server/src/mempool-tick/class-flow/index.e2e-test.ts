...
import MempoolMonitoringClassModel, { AGGREGATE_ID, MempoolTickEvent, MempoolTxSeenEvent } from './mempool.model';
...
describe('/Bitcoin Crawler: Mempool Monitoring Flow (class model)', () => {
  ...
  beforeAll(async () => {
    ...
    await bootstrap({
      Models: [MempoolMonitoringClassModel],
      testing: {
        handlerEventsToWait: [
  ...
  it('should fire mempoolTick — MempoolTickEvent persisted in user model', async () => {
    dbService = new SQLiteService({ path: resolve(process.cwd(), 'eventstore/bitcoin.db') });
    await dbService.connect();
  ...
  it('should iterate ctx.mempool.iterLoadedTx and persist event for each loaded transaction', async () => {
    dbService = new SQLiteService({ path: resolve(process.cwd(), 'eventstore/bitcoin.db') });
    await dbService.connect();
