...
app.get('/unspent', async (req, res) => {
  try {
    const q = (req.query.addresses as string) || '';
    const addresses = q ? q.split(',').map(s => s.trim()).filter(Boolean) : [];
    const data = await client.query<any, any>('GetUnspentQuery', { addresses });
    res.status(200).json(data);
  } catch (e: any) {
    res.status(500).json({ error: e?.message || 'internal' });
  }
...
app.get('/txids', async (req, res) => {
  try {
    const q = (req.query.addresses as string) || '';
    const addresses = q ? q.split(',').map(s => s.trim()).filter(Boolean) : [];
    const data = await client.query<any, any>('GetTxidsQuery', { addresses });
    res.status(200).json(data);
  } catch (e: any) {
    res.status(500).json({ error: e?.message || 'internal' });
  }
...
const server = app.listen(PORT, () => {
  ...
  console.log('curl "http://localhost:' + PORT + '/unspent?addresses=bc1qexampleaddr1...,bc1qexampleaddr2..."');
  console.log('curl "http://localhost:' + PORT + '/txids?addresses=bc1qexampleaddr1...,bc1qexampleaddr2..."');
